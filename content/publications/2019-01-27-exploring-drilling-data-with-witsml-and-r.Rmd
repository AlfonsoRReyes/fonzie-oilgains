...
## The two major nodes
Starting by the top, in the second line, see the first node. `trajectorys`. This is the __root__ node. If you see one `(-)` sign on its left, it means it has been expanded. Following on the right, in red, you see labels such as `xmlns`, `xlmns:xsi`, and `version`. These are __attributes__ of `trajectorys`. They are not nodes but attributes.
...
## Secondary nodes
There is a group of nodes that coexists at the same level of the most important node `called `trajectoryStation`. This group of nodes is what we call siblings of `trajectoryStation`. These nodes are:
...
We will use two packages: `xml2`, `data.table` and `dplyr`. `xml2` will take care of reading the WITSML or XML files, while `dplyr` and `data.table` will be our workhorses for dataframe generation and data manipulation. I could have just used `dplyr` nut wanted to show you both table libraries.
...
As we described succintly above there are ___ groups of nodes that we are interested in:

1. The root node
2. The main node
3. The measurements node and subnodes
4. The siblings of the measurements nodes

Our task is putting together all the variables and their corresponding values in a data structure that we are familiar with: the table. This step may not totally necessary if we have the proper software or tools that do the collection and search in the background for us. For the time being, let's take like it were an anatomy lesson.
...
If we know by specification that a node does not have dependent nodes then we could manually indicate the indices of the names vector. This requires the specification or a manual inspection node by node.
...
Pat attention to these two cases when we interrogate about the name of the variables under `trajectoryStation`.
```{r}
...
> They are different because int he first case we are asking for the first node (`xml_find_first`) while in the second case we are asking for all the nodes (xml_find_all). This is normal, the reason of existence of hierarchical structures that doesn't waste space in variables that are not currently used.

### Attrubutes of `trajectoryStation`
...
There are two type of nodes with the same name:
...
```{r}
trajectory_dt <- cbind(trajectory_dt, siblings_dt)
trajectory_df <- cbind(trajectory_df, siblings_df)
trajectory_df
```
...
### dataframe for `trajectoryStation` attribute `uid`
...
as.tibble(tS.trajectoryStation_df)
```
...
# combine all dataframes to make up trajectoryStation dataframe
  ...
  # tS.cD_df: commonData
trajectoryStation_df <- cbind(tS.uid_dt, tS_df, tS.cU_df, tS.cD_df)
trajectoryStation_dt <- data.table(trajectoryStation_df)
as.tibble(trajectoryStation_df)
```
...
We have one interesting thing going on. The have the trajectory dataframe that is one row and 17 columns, while trajectoryStation is 25 rows by 27 columns. How do we work them out in rectangular tables:
...
### trajectoryStation for the 2nd well
...
Now, we can put all these functions in an R script, and would work much faster. In this example, we are converting the WITSML to dataframe for well *Norway-Statoil-NO 15_$47\$\_9-F-7*. We will call it by the last identifier `9f7` from now on. You will see that this well has more trajectory stations; 43 to be exact. Many more than the first well `9f9` that corresponds to well *Norway-Statoil-NO 15_$47\$\_9-F-9*.
...
## Combine the two wells with `rbind`
Now it's the time to combine both wells
...
## Add new wells
We will add one more well: *Norway-Statoil-NO 15_$47\$\_9-F-4*. This well has 87 measurement points for its trajectory.
